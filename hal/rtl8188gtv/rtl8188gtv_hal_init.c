...
#include "hal8188gtv_fw.h"

#if 0 /* FW tx packet write */
	#define FW_DOWNLOAD_SIZE_8188GTV 8192
#endif
...
)
{
...
#ifdef CONFIG_USB_HCI
		ret = rtw_writeN(padapter, (FW_8188GTV_START_ADDRESS + i * blockSize_p1), blockSize_p1, (bufferPtr + i * blockSize_p1));
#else
		ret = rtw_write32(padapter, (FW_8188GTV_START_ADDRESS + i * blockSize_p1), le32_to_cpu(*((u32 *)(bufferPtr + i * blockSize_p1))));
#endif
		if (ret == _FAIL) {
			printk("====>%s %d i:%d\n", __func__, __LINE__, i);
			goto exit;
	...
	if (remainSize_p1) {
		...
		case 1:
			remainFW[0] = *(p);
			ret = rtw_write32(padapter, (FW_8188GTV_START_ADDRESS + blockCount_p1 * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter, (FW_8188GTV_START_ADDRESS + offset + i * blockSize_p2), blockSize_p2, (bufferPtr + offset + i * blockSize_p2));
	...
	if (remainSize_p2) {
		...
		for (i = 0; i < blockCount_p3; i++) {
			ret = rtw_write8(padapter, (FW_8188GTV_START_ADDRESS + offset + i), *(bufferPtr + offset + i));
...
void rtl8188gtv_FirmwareSelfReset(PADAPTER padapter)
{
	...
	u8 FwBuffer[FW_8188GTV_SIZE];
#endif /* CONFIG_FILE_FWIMG */

#ifdef CONFIG_MP_INCLUDED
int _WriteBTFWtoTxPktBuf8188GTV(
			PADAPTER	Adapter,
			void			*buffer,
			u32			FwBufLen,
			u8			times
)
{
	...
	rtStatus =	InitLLTTable8188GTVS(Adapter, txpktbuf_bndy);
	if (RT_STATUS_SUCCESS != rtStatus) {
		RTW_INFO("_CheckWLANFwPatchBTFwReady_8188GTV(): Failed to init LLT!\n");
		return RT_STATUS_FAILURE;
	}

	/* Init Tx boundary. */
	PlatformEFIOWrite1Byte(Adapter, REG_DWBCN0_CTRL_8188GTV + 1, (u8)txpktbuf_bndy);
#endif
...
exit:
	...
	if (pGenBufReservedPagePacket) {
		RTW_INFO("_WriteBTFWtoTxPktBuf8188GTV => rtw_mfree pGenBufReservedPagePacket!\n");
...
)
{
	...
	SET_8188GTV_H2CCMD_BT_FW_PATCH_SIZE(u1BTFwPatchParm, FwSize);
	SET_8188GTV_H2CCMD_BT_FW_PATCH_ADDR0(u1BTFwPatchParm, addr0);
	SET_8188GTV_H2CCMD_BT_FW_PATCH_ADDR1(u1BTFwPatchParm, addr1);
	SET_8188GTV_H2CCMD_BT_FW_PATCH_ADDR2(u1BTFwPatchParm, addr2);
	SET_8188GTV_H2CCMD_BT_FW_PATCH_ADDR3(u1BTFwPatchParm, addr3);

	FillH2CCmd8188GTV(Adapter, H2C_8188GTV_BT_FW_PATCH, H2C_BT_FW_PATCH_LEN, u1BTFwPatchParm);
...
)
{
	...
	SET_8188GTV_H2CCMD_BT_PWR_IDX(u1BTPwrIdxParm, PwrIdx);


	FillH2CCmd8188GTV(Adapter, H2C_8188GTV_FORCE_BT_TXPWR, H2C_FORCE_BT_TXPWR_LEN, u1BTPwrIdxParm);
}
...
)
{
	...
	/*--------------------------------------------------------- */
	do {
		u1bTmp = PlatformEFIORead1Byte(Adapter, REG_HMEBOX_DBG_0_8188GTV);
	...
	u1bTmp = PlatformEFIORead1Byte(Adapter, REG_CR_8188GTV + 1);
	PlatformEFIOWrite1Byte(Adapter, REG_CR_8188GTV + 1, (u1bTmp & (~BIT0)));
 ...
 * into four parts to transfer. The last parameter of _WriteBTFWtoTxPktBuf8188GTV is used to indicate
...
s32 FirmwareDownloadBT(PADAPTER padapter, PRT_MP_FIRMWARE pFirmware)
{
	...
	if (rtw_is_file_readable(rtw_fw_mp_bt_file_path) == _TRUE) {
		...
		rtStatus = rtw_retrieve_from_file(rtw_fw_mp_bt_file_path, FwBuffer, FW_8188GTV_SIZE);
...
#endif /* CONFIG_FILE_FWIMG */
	{
		...
		pBTFirmwareBuf = (u8 *)Rtl8188GTVFwBTImgArray;
		BTFirmwareLen = Rtl8188GTVFwBTImgArrayLength;
	...
	for (i = (download_time - 1); i >= 0; i--) {
		if (i == (download_time - 1)) {
			rtStatus = _WriteBTFWtoTxPktBuf8188GTV(padapter, pBTFirmwareBuf + (4096 * i), (BTFirmwareLen - (4096 * i)), 1);
			RTW_INFO("%s: start %d, len %d, time 1\n", __func__, 4096 * i, BTFirmwareLen - (4096 * i));
		} else {
			rtStatus = _WriteBTFWtoTxPktBuf8188GTV(padapter, pBTFirmwareBuf + (4096 * i), 4096, (download_time - i));
...
void rtl8188gtv_cal_txdesc_chksum(struct tx_desc *ptxdesc)
{
...
u8 send_fw_packet(PADAPTER padapter, u8 *pRam_code, u32 length)
{
	...
	if (FW_DOWNLOAD_SIZE_8188GTV == length)
		length--;
...
#else
	SET_TX_DESC_QUEUE_SEL_8188GTV(pTmp_buffer, QSLT_BEACON);
	SET_TX_DESC_PKT_SIZE_8188GTV(pTmp_buffer, modify_ram_size - TXDESC_SIZE);
	SET_TX_DESC_OFFSET_8188GTV(pTmp_buffer, TXDESC_SIZE);
#endif
	rtl8188gtv_cal_txdesc_chksum((struct tx_desc *)pTmp_buffer);
	...
	do {
		...
		value8 = rtw_read8(padapter, REG_DWBCN0_CTRL_8188GTV + 2);
		counter--;
	} while (0 == (value8 & BIT(0)));

	rtw_write8(padapter, REG_DWBCN0_CTRL_8188GTV + 2, value8);
...
s32 rtl8188gtv_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	PRT_FIRMWARE_8188GTV	pFirmware = NULL;
	PRT_8188GTV_FIRMWARE_HDR		pFwHdr = NULL;
	u8			*pFirmwareBuf;
	u32			FirmwareLen;
#ifdef CONFIG_FILE_FWIMG
	u8 *fwfilepath;
	...
	pFirmware = (PRT_FIRMWARE_8188GTV)rtw_zmalloc(sizeof(RT_FIRMWARE_8188GTV));
...
#ifdef CONFIG_FILE_FWIMG
		rtStatus = rtw_retrieve_from_file(fwfilepath, FwBuffer, FW_8188GTV_SIZE);
	...
	case FW_SOURCE_HEADER_FILE:
		if (bUsedWoWLANFw) {
			...
			if (pwrpriv->wowlan_mode) {
				pFirmware->szFwBuffer = array_mp_8188gtv_fw_wowlan;
				pFirmware->ulFwLength = array_length_mp_8188gtv_fw_wowlan;
			...
			if (pwrpriv->wowlan_ap_mode) {
				pFirmware->szFwBuffer = array_mp_8188gtv_fw_ap;
				pFirmware->ulFwLength = array_length_mp_8188gtv_fw_ap;
			...
			pFirmware->szFwBuffer = array_mp_8188gtv_fw_nic;
			pFirmware->ulFwLength = array_length_mp_8188gtv_fw_nic;
			RTW_INFO("%s fw: %s, size: %d\n", __func__
				/*, "FW_WoWLAN" */
				, "FW_NIC"
				, pFirmware->ulFwLength);
	...
	if ((pFirmware->ulFwLength - 32) > FW_8188GTV_SIZE) {
		rtStatus = _FAIL;
		RTW_ERR("Firmware size:%u exceed %u\n", pFirmware->ulFwLength, FW_8188GTV_SIZE);
		goto exit;
	}
	...
	pFwHdr = (PRT_8188GTV_FIRMWARE_HDR)pFirmwareBuf;
	...
	if (IS_FW_HEADER_EXIST_8188GTV(pFwHdr)) {
	...
	*/
	...
	if ((rtw_read8(padapter, REG_MCUFWDL) & MCUFWDL_RDY) == 0) {
		...
		rtw_write16(padapter, REG_DWBCN0_CTRL_8188GTV + 1, BIT(8));
		...
		while (0 != pkt_size_tmp) {
			if (pkt_size_tmp >= FW_DOWNLOAD_SIZE_8188GTV) {
				send_pkt_size = FW_DOWNLOAD_SIZE_8188GTV;
...
exit:
	if (pFirmware)
		rtw_mfree((u8 *)pFirmware, sizeof(RT_FIRMWARE_8188GTV));

	rtl8188gtv_InitializeFirmwareVars(padapter);
	...
	u8			bPseudoTest)
{
	...
	case TYPE_EFUSE_MAX_SECTION: {
		...
		if (efuseType == EFUSE_WIFI)
			*pMax_section = EFUSE_MAX_SECTION_8188GTV;
		else
	...
	case TYPE_EFUSE_REAL_CONTENT_LEN: {
		...
		if (efuseType == EFUSE_WIFI)
			*pu2Tmp = EFUSE_REAL_CONTENT_LEN_8188GTV;
		else
	...
	case TYPE_AVAILABLE_EFUSE_BYTES_BANK: {
		...
		if (efuseType == EFUSE_WIFI)
			*pu2Tmp = (EFUSE_REAL_CONTENT_LEN_8188GTV - EFUSE_OOB_PROTECT_BYTES);
		else
	...
	u8			bPseudoTest)
{
	...
	while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
		...
		if (offset < EFUSE_MAX_SECTION_8188GTV) {
			u16 addr;
...
)
{
	...
	for (bank = 1; bank < 3; bank++) { /* 8188GTV Max bake 0~2 */
		...
		while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
			...
			if (efuseHeader == 0xFF)
				break;
			RTW_INFO("%s: efuse[%#X]=0x%02x (header)\n", __func__, (((bank - 1) * EFUSE_REAL_CONTENT_LEN_8188GTV) + eFuse_Addr - 1), efuseHeader);
			...
			if (EXT_HEADER(efuseHeader)) {	/*extended header */
				...
				/*ReadEFuseByte(padapter, eFuse_Addr++, &efuseExtHdr, bPseudoTest); */
				efuse_OneByteRead(padapter, eFuse_Addr++, &efuseExtHdr, bPseudoTest);
				RTW_INFO("%s: efuse[%#X]=0x%02x (ext header)\n", __func__, (((bank - 1) * EFUSE_REAL_CONTENT_LEN_8188GTV) + eFuse_Addr - 1), efuseExtHdr);
				if (ALL_WORDS_DISABLED(efuseExtHdr))
					continue;
...
static void rtl8188gtv_read_chip_version(PADAPTER padapter)
{
	u32 value32;
	...
	pHalData->version_id.ICType = CHIP_8188GTV;
...
void rtl8188gtv_InitBeaconParameters(PADAPTER padapter)
{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) == _FALSE)
		rtw_write8(padapter, REG_DRVERLYINT, DRIVER_EARLY_INT_TIME_8188GTV); /* 5ms */
	rtw_write8(padapter, REG_BCNDMATIM, BCN_DMA_ATIME_INT_TIME_8188GTV); /* 2ms */
...
void _InitBurstPktLen_8188GTVS(PADAPTER Adapter)
{
	...
	rtw_write8(Adapter, REG_RX_PKT_LIMIT_8188GTV, 0x18);		/*for VHT packet length 11K */
	rtw_write8(Adapter, REG_MAX_AGGR_NUM_8188GTV, 0x1F);
	rtw_write8(Adapter, REG_PIFS_8188GTV, 0x00);
	rtw_write8(Adapter, REG_FWHW_TXQ_CTRL_8188GTV, rtw_read8(Adapter, REG_FWHW_TXQ_CTRL) & (~BIT(7)));
	if (pHalData->AMPDUBurstMode)
		rtw_write8(Adapter, REG_AMPDU_BURST_MODE_8188GTV,  0x5F);
	rtw_write8(Adapter, REG_AMPDU_MAX_TIME_8188GTV, 0x70);

	/* ARFB table 9 for 11ac 5G 2SS */
	rtw_write32(Adapter, REG_ARFR0_8188GTV, 0x00000010);
	if (IS_NORMAL_CHIP(pHalData->version_id))
		rtw_write32(Adapter, REG_ARFR0_8188GTV + 4, 0xfffff000);
	else
		rtw_write32(Adapter, REG_ARFR0_8188GTV + 4, 0x3e0ff000);

	/* ARFB table 10 for 11ac 5G 1SS */
	rtw_write32(Adapter, REG_ARFR1_8188GTV, 0x00000010);
	rtw_write32(Adapter, REG_ARFR1_8188GTV + 4, 0x003ff000);
}
...
static void rtl8188gtv_SetBeaconRelatedRegisters(PADAPTER padapter)
{
	u8 val8;
	u32 value32;
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	...
	rtl8188gtv_InitBeaconParameters(padapter);
...
void hal_notch_filter_8188gtv(_adapter *adapter, bool enable)
{
	if (enable) {
...
u8 rtl8188gtv_MRateIdxToARFRId(PADAPTER padapter, u8 rate_idx)
{
	u8 ret = 0;
...
void rtl8188gtv_fill_fake_txdesc(
	PADAPTER	padapter,
	u8			*pDesc,
	u32			BufferLen,
	u8			IsPsPoll,
	...
	u8			bDataFrame)
{
	...
	SET_TX_DESC_FIRST_SEG_8188GTV(pDesc, 1); /*bFirstSeg; */
	SET_TX_DESC_LAST_SEG_8188GTV(pDesc, 1); /*bLastSeg; */

	SET_TX_DESC_OFFSET_8188GTV(pDesc, 0x28); /* Offset = 32 */

	SET_TX_DESC_PKT_SIZE_8188GTV(pDesc, BufferLen); /* Buffer size + command header */
	SET_TX_DESC_QUEUE_SEL_8188GTV(pDesc, QSLT_MGNT); /* Fixed queue of Mgnt queue */

	/* Set NAVUSEHDR to prevent Ps-poll AId filed to be changed to error vlaue by Hw. */
	if (_TRUE == IsPsPoll) {
		/* Nothing */
		SET_TX_DESC_NAV_USE_HDR_8188GTV(pDesc, 1);
	} else {
		SET_TX_DESC_HWSEQ_EN_8188GTV(pDesc, 1); /* Hw set sequence number */
		SET_TX_DESC_HWSEQ_SEL_8188GTV(pDesc, 0);
	}

	if (_TRUE == IsBTQosNull)
		SET_TX_DESC_BT_INT_8188GTV(pDesc, 1);

	SET_TX_DESC_USE_RATE_8188GTV(pDesc, 1); /* use data rate which is set by Sw */
	SET_TX_DESC_OWN_8188GTV((u8 *)pDesc, 1);

	SET_TX_DESC_TX_RATE_8188GTV(pDesc, DESC8188GTV_RATE1M);
	...
	if (_TRUE == bDataFrame) {
		...
		switch (EncAlg) {
		case _NO_PRIVACY_:
			SET_TX_DESC_SEC_TYPE_8188GTV(pDesc, 0x0);
			break;
		case _WEP40_:
		case _WEP104_:
		case _TKIP_:
			SET_TX_DESC_SEC_TYPE_8188GTV(pDesc, 0x1);
			break;
		case _SMS4_:
			SET_TX_DESC_SEC_TYPE_8188GTV(pDesc, 0x2);
			break;
		case _AES_:
			SET_TX_DESC_SEC_TYPE_8188GTV(pDesc, 0x3);
			break;
		default:
			SET_TX_DESC_SEC_TYPE_8188GTV(pDesc, 0x0);
			break;
	...
	rtl8188gtv_cal_txdesc_chksum((struct tx_desc *)pDesc);
#endif
}
...
void _DisableRFAFEAndResetBB8188GTV(PADAPTER padapter)
{
	/*
...
{
	_DisableRFAFEAndResetBB8188GTV(padapter);
}

void _ResetDigitalProcedure1_8188GTV(PADAPTER padapter, BOOLEAN bWithoutHWSM)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
...
{
	_ResetDigitalProcedure1_8188GTV(padapter, bWithoutHWSM);
}
	...
	u8			*PROMContent)
{
	...
	if (_FALSE == pHalData->bautoload_fail_flag) {
		...
		if (_TRUE == pHalData->EepromOrEfuse) {
			...
			for (i = 0; i < HWSET_MAX_SIZE_8188GTV; i += 2) {
			...
			_rtw_memcpy((void *)PROMContent, (void *)pHalData->efuse_eeprom_data, HWSET_MAX_SIZE_8188GTV);
		}
	} else {
		/*autoload fail */
...
void
Hal_EfuseParseTxPowerInfo_8188GTV(
		PADAPTER		padapter,
		u8			*PROMContent,
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->EEPROMRegulatory = (PROMContent[EEPROM_RF_BOARD_OPTION_8188GTV] & 0x7);	/*bit0~2 */
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8188GTV] == 0xFF)
...
void
Hal_EfuseParseEEPROMVer_8188GTV(
		PADAPTER		padapter,
		u8			*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	...
	if (!AutoLoadFail)
		pHalData->EEPROMVersion = hwinfo[EEPROM_VERSION_8188GTV];
	else
...
)
{
	...
	/*_rtw_memcpy(pEEPROM->adjuseVoltageVal, &hwinfo[EEPROM_Voltage_ADDR_8188GTV], 1); */
	RTW_INFO("%s hwinfo[EEPROM_Voltage_ADDR_8188GTV] =%02x\n", __func__, hwinfo[EEPROM_Voltage_ADDR_8188GTV]);
	pHalData->adjuseVoltageVal = (hwinfo[EEPROM_Voltage_ADDR_8188GTV] & 0xf0) >> 4;
	RTW_INFO("%s pHalData->adjuseVoltageVal =%x\n", __func__, pHalData->adjuseVoltageVal);
}
#endif

void
Hal_EfuseParseChnlPlan_8188GTV(
		PADAPTER		padapter,
		u8			*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	hal_com_config_channel_plan(
		padapter
		, hwinfo ? &hwinfo[EEPROM_COUNTRY_CODE_8188GTV] : NULL
		, hwinfo ? hwinfo[EEPROM_ChannelPlan_8188GTV] : 0xFF
...
void
Hal_EfuseParseCustomerID_8188GTV(
		PADAPTER		padapter,
		u8			*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	...
	if (!AutoLoadFail)
		pHalData->EEPROMCustomerID = hwinfo[EEPROM_CustomID_8188GTV];
	else
		pHalData->EEPROMCustomerID = 0;
}

void
Hal_EfuseParsePowerSavingMode_8188GTV(
		PADAPTER		padapter,
		u8				*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	...
	if (AutoLoadFail) {
		...
		if (padapter->registrypriv.hwpdn_mode == 2)
			pwrctl->bHWPowerdown = (hwinfo[EEPROM_FEATURE_OPTION_8188GTV] & BIT4);
		else
...
#ifdef CONFIG_USB_HCI
		pwrctl->bSupportRemoteWakeup = (hwinfo[EEPROM_USB_OPTIONAL_FUNCTION0_8188GTVU] & BIT1) ? _TRUE : _FALSE;
...
void
Hal_EfuseParseAntennaDiversity_8188GTV(
		PADAPTER		pAdapter,
		u8				*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	...
	if (registry_par->antdiv_cfg == 2) {
		if (0x01 == hwinfo[EEPROM_RF_ANTENNA_OPT_8188GTV])
...
#if 0
		pHalData->TRxAntDivType = hwinfo[EEPROM_RFE_OPTION_8188GTV];
		if (pHalData->TRxAntDivType == 0xFF)
			pHalData->TRxAntDivType = S0S1_SW_ANTDIV;
		else
			RTW_INFO("%s: efuse[0x%x]=0x%02x is unknown type\n",
				__func__, EEPROM_RFE_OPTION_8188GTV, pHalData->TRxAntDivType);
#else
		/* 8188GTV only intrnal switch S0S1 */
		pHalData->TRxAntDivType = S0S1_SW_ANTDIV;
#endif
	} else {
		/* 8188GTV only intrnal switch S0S1 */
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->crystal_cap = hwinfo[EEPROM_XTAL_8188GTV];
		if (pHalData->crystal_cap == 0xFF)
			pHalData->crystal_cap = EEPROM_Default_CrystalCap_8188GTV;	   /*what value should 8812 set? */
	} else
		pHalData->crystal_cap = EEPROM_Default_CrystalCap_8188GTV;
}


void
Hal_EfuseParseThermalMeter_8188GTV(
	PADAPTER	padapter,
	u8			*PROMContent,
	u8			AutoLoadFail
)
{
	...
	if (_FALSE == AutoLoadFail)
		pHalData->eeprom_thermal_meter = PROMContent[EEPROM_THERMAL_METER_8188GTV];
	else
		pHalData->eeprom_thermal_meter = EEPROM_Default_ThermalMeter_8188GTV;
...
void Hal_EfuseParseKFreeData_8188GTV(
			PADAPTER		Adapter,
			u8				*PROMContent,
			BOOLEAN		AutoloadFail)
{
#ifdef CONFIG_RF_POWER_TRIM
#define THERMAL_K_MEAN_OFFSET_8188GTV 5 /* 8188GTV FT thermal K mean value has +5 offset, it's special case */

	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	struct kfree_data_t *kfree_data = &pHalData->kfree_data;
	u8 pg_pwrtrim = 0xFF, pg_therm = 0xFF;

	efuse_OneByteRead(Adapter, PPG_BB_GAIN_2G_TXA_OFFSET_8188GTV, &pg_pwrtrim, _FALSE);
	efuse_OneByteRead(Adapter, PPG_THERMAL_OFFSET_8188GTV ,&pg_therm ,_FALSE);
	...
	if (pg_therm != 0xFF) {
		kfree_data->thermal
			= KFREE_THERMAL_OFFSET(pg_therm  & PPG_THERMAL_OFFSET_MASK) - THERMAL_K_MEAN_OFFSET_8188GTV;
		if (GET_PG_KFREE_THERMAL_K_ON_8188GTV(PROMContent))
...
u8	SCMapping_8188GTV(PADAPTER Adapter, struct pkt_attrib *pattrib)
{
...
void fill_txdesc_force_bmc_camid(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && (!pattrib->bswenc)
		...
		SET_TX_DESC_EN_DESC_ID_8188GTV(ptxdesc, 1);
		SET_TX_DESC_MACID_8188GTV(ptxdesc, pattrib->bmc_camid);
	}
}
#endif
...
static void fill_txdesc_vcs_8188gtv(PADAPTER padapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	/*RTW_INFO("cvs_mode=%d\n", pattrib->vcs_mode); */

	SET_TX_DESC_HW_RTS_ENABLE_8188GTV(ptxdesc, 0);

	switch (pattrib->vcs_mode) {
	case RTS_CTS:
		SET_TX_DESC_RTS_ENABLE_8188GTV(ptxdesc, 1);
		break;
	case CTS_TO_SELF:
		SET_TX_DESC_CTS2SELF_8188GTV(ptxdesc, 1);
		break;
	case NONE_VCS:
	default:
		break;
	...
	if (pattrib->vcs_mode) {
		/* SET_TX_DESC_CCA_RTS_8188GTV(ptxdesc, pTcb->RTSCCA); */
		SET_TX_DESC_RTS_RATE_8188GTV(ptxdesc, 8); /*TODO: Hardcode?, RTS Rate=24M (8) */
		SET_TX_DESC_RTS_RATE_FB_LIMIT_8188GTV(ptxdesc, 0xF); /* TODO: PROTECTION_MODE ?? */

		if (padapter->mlmeextpriv.mlmext_info.preamble_mode == PREAMBLE_SHORT)
			SET_TX_DESC_RTS_SHORT_8188GTV(ptxdesc, 1);

		/* Set RTS BW */
		if (pattrib->ht_en)
			SET_TX_DESC_RTS_SC_8188GTV(ptxdesc, SCMapping_8188GTV(padapter, pattrib));
	}
#endif
}

static void fill_txdesc_phy_8188gtv(PADAPTER padapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	/*RTW_INFO("bwmode=%d, ch_off=%d\n", pattrib->bwmode, pattrib->ch_offset); */

	if (pattrib->ht_en) {
		SET_TX_DESC_DATA_BW_8188GTV(ptxdesc, BWMapping_8188GTV(padapter, pattrib));
		SET_TX_DESC_DATA_SC_8188GTV(ptxdesc, SCMapping_8188GTV(padapter, pattrib));
	}
}

static void rtl8188gtv_fill_default_txdesc(
	...
	u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		SET_TX_DESC_MACID_8188GTV(pbuf, pattrib->mac_id);
		SET_TX_DESC_RATE_ID_8188GTV(pbuf, pattrib->raid);
		SET_TX_DESC_QUEUE_SEL_8188GTV(pbuf, pattrib->qsel);
		SET_TX_DESC_SEQ_8188GTV(pbuf, pattrib->seqnum);

		SET_TX_DESC_SEC_TYPE_8188GTV(pbuf, fill_txdesc_sectype(pattrib));
#if defined(CONFIG_CONCURRENT_MODE)
		if (bmcst)
			fill_txdesc_force_bmc_camid(pattrib, pbuf);
#endif
		fill_txdesc_vcs_8188gtv(padapter, pattrib, pbuf);
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				SET_TX_DESC_AGG_ENABLE_8188GTV(pbuf, 1);
				SET_TX_DESC_MAX_AGG_NUM_8188GTV(pbuf, 0x1F);
				SET_TX_DESC_AMPDU_DENSITY_8188GTV(pbuf, pattrib->ampdu_spacing);
			} else
				SET_TX_DESC_AGG_BREAK_8188GTV(pbuf, 1);

			fill_txdesc_phy_8188gtv(padapter, pattrib, pbuf);

			SET_TX_DESC_DATA_RATE_FB_LIMIT_8188GTV(pbuf, 0x1F);

			if (pHalData->fw_ractrl == _FALSE) {
				SET_TX_DESC_USE_RATE_8188GTV(pbuf, 1);

				if (pHalData->INIDATA_RATE[pattrib->mac_id] & BIT(7))
					SET_TX_DESC_DATA_SHORT_8188GTV(pbuf, 1);

				SET_TX_DESC_TX_RATE_8188GTV(pbuf, pHalData->INIDATA_RATE[pattrib->mac_id] & 0x7F);
			}
			if (bmcst) {
				SET_TX_DESC_USE_RATE_8188GTV(pbuf, 1);
				SET_TX_DESC_TX_RATE_8188GTV(pbuf, MRateToHwRate(pattrib->rate));
				SET_TX_DESC_DISABLE_FB_8188GTV(pbuf, 1);
			}
			...
			if (padapter->fix_rate != 0xFF) {
				...
				if (padapter->fix_rate & BIT(7))
					SET_TX_DESC_DATA_SHORT_8188GTV(pbuf, 1);
				SET_TX_DESC_TX_RATE_8188GTV(pbuf, padapter->fix_rate & 0x7F);
				if (!padapter->data_fb)
					SET_TX_DESC_DISABLE_FB_8188GTV(pbuf, 1);
			}

			if (pattrib->ldpc)
				SET_TX_DESC_DATA_LDPC_8188GTV(pbuf, 1);

			if (pattrib->stbc)
				SET_TX_DESC_DATA_STBC_8188GTV(pbuf, 1);

#ifdef CONFIG_CMCC_TEST
			SET_TX_DESC_DATA_SHORT_8188GTV(pbuf, 1); /* use cck short premble */
#endif
		} else {
			...
			if((pattrib->ether_type == ETH_P_ARP) &&
				...
				SET_TX_DESC_TX_RATE_8188GTV(pbuf, MRateToHwRate(IEEE80211_OFDM_RATE_6MB));
			 else
#endif/*CONFIG_IP_R_MONITOR*/
				SET_TX_DESC_TX_RATE_8188GTV(pbuf, MRateToHwRate(pmlmeext->tx_rate));
		...
		SET_TX_DESC_USB_TXAGG_NUM_8188GTV(pbuf, pxmitframe->agg_num);
#endif
...
#endif
			SET_TX_DESC_SPE_RPT_8188GTV(pbuf, 1);
			SET_TX_DESC_SW_DEFINE_8188GTV(pbuf, (u8)(GET_PRIMARY_ADAPTER(padapter)->xmitpriv.seq_no));
		}
		...
		SET_TX_DESC_MBSSID_8188GTV(pbuf, pattrib->mbssid & 0xF);

		SET_TX_DESC_RETRY_LIMIT_ENABLE_8188GTV(pbuf, 1);
		if (pattrib->retry_ctrl == _TRUE) {
			/* Nothing */
			SET_TX_DESC_DATA_RETRY_LIMIT_8188GTV(pbuf, 6);
		} else {
			/* Nothing */
			SET_TX_DESC_DATA_RETRY_LIMIT_8188GTV(pbuf, 12);
		}
	...
	SET_TX_DESC_PKT_SIZE_8188GTV(pbuf, pattrib->last_txcmdsz);

	{
		...
		SET_TX_DESC_PKT_OFFSET_8188GTV(pbuf, pkt_offset);
		SET_TX_DESC_OFFSET_8188GTV(pbuf, offset);
	}

	if (bmcst)
		SET_TX_DESC_BMC_8188GTV(pbuf, 1);
	...
	/* 2010.06.23. Added by tynli. */
	if (!pattrib->qos_en)
		SET_TX_DESC_HWSEQ_EN_8188GTV(pbuf, 1);
 ...
 */
void rtl8188gtv_update_txdesc(struct xmit_frame *pxmitframe, u8 *pbuf)
{
	rtl8188gtv_fill_default_txdesc(pxmitframe, pbuf);

#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI)
	rtl8188gtv_cal_txdesc_chksum((struct tx_desc *)pbuf);
#endif
}
...
static void hw_var_set_opmode(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	if (padapter->hw_port == HW_PORT1) {
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
...
#ifdef CONFIG_PCI_HCI
				UpdateInterruptMask8188GTVE(padapter, 0, 0, RT_BCN_INT_MASKS, 0);
#else /* !CONFIG_PCI_HCI */
#ifdef CONFIG_INTERRUPT_BASED_TXBCN

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT
				rtw_write8(padapter, REG_DRVERLYINT, 0x05);/*restore early int time to 5ms */
				UpdateInterruptMask8188GTVU(padapter, _TRUE, 0, IMR_BCNDMAINT0_8188GTV);
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
				UpdateInterruptMask8188GTVU(padapter, _TRUE , 0, (IMR_TXBCN0ERR_8188GTV | IMR_TXBCN0OK_8188GTV));
...
#ifdef CONFIG_PCI_HCI
			UpdateInterruptMask8188GTVE(padapter, RT_BCN_INT_MASKS, 0, 0, 0);
#else /* !CONFIG_PCI_HCI */
#ifdef CONFIG_INTERRUPT_BASED_TXBCN

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT
			UpdateInterruptMask8188GTVU(padapter, _TRUE, IMR_BCNDMAINT0_8188GTV, 0);
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
			UpdateInterruptMask8188GTVU(padapter, _TRUE, (IMR_TXBCN0ERR_8188GTV | IMR_TXBCN0OK_8188GTV), 0);
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR */
			...
			rtw_write8(padapter, REG_CCK_CHECK_8188GTV,
				(rtw_read8(padapter, REG_CCK_CHECK_8188GTV) | BIT_BCN_PORT_SEL));
...
#ifdef CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT
				rtw_write8(padapter, REG_DRVERLYINT, 0x05); /* restore early int time to 5ms */
				UpdateInterruptMask8812AU(padapter, _TRUE, 0, IMR_BCNDMAINT0_8188GTV);
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
				UpdateInterruptMask8812AU(padapter, _TRUE , 0, (IMR_TXBCN0ERR_8188GTV | IMR_TXBCN0OK_8188GTV));
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR */
...
#define	GET_8188GTV_C2H_TX_RPT_LIFE_TIME_OVER(_Header)	LE_BITS_TO_1BYTE((_Header + 0), 6, 1)
#define	GET_8188GTV_C2H_TX_RPT_RETRY_OVER(_Header)	LE_BITS_TO_1BYTE((_Header + 0), 7, 1)
...
#ifdef CONFIG_XMIT_ACK
	if (GET_8188GTV_C2H_TX_RPT_RETRY_OVER(pdata) | GET_8188GTV_C2H_TX_RPT_LIFE_TIME_OVER(pdata))
...
static s32 c2h_handler_8188gtv(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	s32 ret = _SUCCESS;

	switch (id) {
	case C2H_CCX_TX_RPT:
		CCX_FwC2HTxRpt_8188gtv(adapter, payload, plen);
		break;
	default:
		ret = _FAIL;
		break;
...
void rtl8188gtv_set_pll_ref_clk_sel(_adapter *adapter, u8 sel)
{
	...
	value8 = rtw_read8(adapter, REG_MAC_PLL_CTRL_EXT_8188GTV);
	if ((value8 & 0x0F) != (sel & 0x0F)) {
		u16 value16;
		u8 ori_bit_wlock_2c;

		value16 = rtw_read16(adapter, REG_RSV_CTRL_8188GTV);
		ori_bit_wlock_2c = (value16 & BIT8) ? 1 : 0;
		if (ori_bit_wlock_2c) {
			value16 &= ~BIT8;
			rtw_write16(adapter, REG_RSV_CTRL_8188GTV, value16);
		}

		RTW_PRINT("switch pll_ref_clk_sel from 0x%x to 0x%x\n"
			  , (value8 & 0x0F), (sel & 0x0F));
		value8 = (value8 & 0xF0) | (sel & 0x0F);
		rtw_write8(adapter, REG_MAC_PLL_CTRL_EXT_8188GTV, value8);
...
u8 SetHwReg8188GTV(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_AMPDU_FACTOR: {
		...
		rtw_write32(padapter, REG_AMPDU_MAX_LENGTH_8188GTV, AMPDULen);
	}
	break;
	...
	case HW_VAR_H2C_FW_PWRMODE: {
		...
		/*	rtl8188gtv_set_lowpwr_lps_cmd(padapter, _TRUE); */
		/*} else { */
		/*	rtl8188gtv_set_lowpwr_lps_cmd(padapter, _FALSE); */
		/*} */
		rtl8188gtv_set_FwPwrMode_cmd(padapter, psmode);
	}
	break;
	case HW_VAR_H2C_PS_TUNE_PARAM:
		rtl8188gtv_set_FwPsTuneParam_cmd(padapter);
		break;

	case HW_VAR_H2C_FW_JOINBSSRPT:
		rtl8188gtv_set_FwJoinBssRpt_cmd(padapter, *val);
		break;

	case HW_VAR_DL_RSVD_PAGE:
#ifdef CONFIG_BT_COEXIST
		if (check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
			rtl8188gtv_download_BTCoex_AP_mode_rsvd_page(padapter);
		else
#endif /* CONFIG_BT_COEXIST */
		{
			rtl8188gtv_download_rsvd_page(padapter, RT_MEDIA_CONNECT);
		}
		break;

#ifdef CONFIG_P2P
	case HW_VAR_H2C_FW_P2P_PS_OFFLOAD:
		rtl8188gtv_set_p2p_ps_offload_cmd(padapter, *val);
		break;
#endif /*CONFIG_P2P */
...
#define RXDMA_IDLE			BIT(17)
	...
	case HW_VAR_NAV_UPPER: {
		...
		if (usNavUpper > HAL_NAV_UPPER_UNIT_8188GTV * 0xFF) {
			break;
		}

		/* The value of ((usNavUpper + HAL_NAV_UPPER_UNIT_8188GTV - 1) / HAL_NAV_UPPER_UNIT_8188GTV) */
		/* is getting the upper integer. */
		usNavUpper = (usNavUpper + HAL_NAV_UPPER_UNIT_8188GTV - 1) / HAL_NAV_UPPER_UNIT_8188GTV;
...
#ifdef CONFIG_CONCURRENT_MODE
		if (padapter->hw_port == HW_PORT1) {
			val8 = rtw_read8(padapter,  REG_DWBCN1_CTRL_8188GTV + 2);
			val8 |= BIT(0);
			rtw_write8(padapter, REG_DWBCN1_CTRL_8188GTV + 2, val8);
		} else
#endif /* CONFIG_CONCURRENT_MODE */
		{
	...
	case HW_VAR_TDLS_BCN_EARLY_C2H_RPT:
		rtl8188gtv_set_BcnEarly_C2H_Rpt_cmd(padapter, *val);
		break;
#endif
	default:
...
struct bcn_qinfo_8188gtv {
	u16 head:8;
	u16 pkt_num:8;
};

void dump_qinfo_8188gtv(void *sel, struct qinfo_8188gtv *info, const char *tag)
{
	/*if (info->pkt_num) */
...
void dump_bcn_qinfo_8188gtv(void *sel, struct bcn_qinfo_8188gtv *info, const char *tag)
{
	/*if (info->pkt_num) */
...
void dump_mac_qinfo_8188gtv(void *sel, _adapter *adapter)
{
	u32 q0_info;
	u32 q1_info;
	u32 q2_info;
	...
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q0_info, "Q0 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q1_info, "Q1 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q2_info, "Q2 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q3_info, "Q3 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q4_info, "Q4 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q5_info, "Q5 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q6_info, "Q6 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&q7_info, "Q7 ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&mg_q_info, "MG ");
	dump_qinfo_8188gtv(sel, (struct qinfo_8188gtv *)&hi_q_info, "HI ");
	dump_bcn_qinfo_8188gtv(sel, (struct bcn_qinfo_8188gtv *)&bcn_q_info, "BCN ");
}

static void dump_mac_txfifo_8188gtv(void *sel, _adapter *adapter)
{
	u32 rqpn, rqpn_npq;
...
void rtl8188gtv_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	GetHwReg8188GTV(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	GetHwReg8188GTV(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	GetHwReg8188GTV(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	GetHwReg8188GTV(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void GetHwReg8188GTV(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_DUMP_MAC_QUEUE_INFO:
		dump_mac_qinfo_8188gtv(val, padapter);
		break;
	case HW_VAR_DUMP_MAC_TXFIFO:
		dump_mac_txfifo_8188gtv(val, padapter);
		break;
	default:
...
void hal_ra_info_dump(_adapter *padapter , void *sel)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if (rtw_macid_is_used(macid_ctl, i) && !rtw_macid_is_bmc(macid_ctl, i)) {
			...
			rtw_write32(padapter, REG_HMEBOX_DBG_2_8188GTV, cmd);
			rtw_msleep_os(10);
...
u8 GetHalDefVar8188GTV(PADAPTER padapter, HAL_DEF_VARIABLE variable, void *pval)
{
	...
	case HAL_DEF_TX_PAGE_BOUNDARY:
		if (!padapter->registrypriv.wifi_spec)
			*(u8 *)pval = TX_PAGE_BOUNDARY_8188GTV;
		else
			*(u8 *)pval = WMM_NORMAL_TX_PAGE_BOUNDARY_8188GTV;
		break;
	case HAL_DEF_TX_PAGE_SIZE:
		*((u32 *)pval) = PAGE_SIZE_128;
		break;
	case HAL_DEF_RX_DMA_SZ_WOW:
		*(u32 *)pval = RX_DMA_SIZE_8188GTV - RESV_FMWF;
		break;
	case HAL_DEF_RX_DMA_SZ:
		*(u32 *)pval = RX_DMA_BOUNDARY_8188GTV + 1;
		break;
...
void rtl8188gtv_start_thread(_adapter *padapter)
{
	...
	if (xmitpriv->SdioXmitThread == NULL) {
		...
		xmitpriv->SdioXmitThread = kthread_run(rtl8188gtvs_xmit_thread, padapter, "RTWHALXT");
		if (IS_ERR(xmitpriv->SdioXmitThread)) {
			RTW_ERR("%s: start rtl8188gtvs_xmit_thread FAIL!!\n", __func__);
...
{
	pHalFunc->dm_init = &rtl8188gtv_init_dm_priv;
	pHalFunc->dm_deinit = &rtl8188gtv_deinit_dm_priv;

	pHalFunc->read_chip_version = &rtl8188gtv_read_chip_version;

	pHalFunc->set_chnl_bw_handler = &PHY_SetSwChnlBWMode8188GTV;

	pHalFunc->set_tx_power_level_handler = &PHY_SetTxPowerLevel8188GTV;
	pHalFunc->set_txpwr_done = rtl8188gtv_set_txpwr_done;
	pHalFunc->set_tx_power_index_handler = PHY_SetTxPowerIndex_8188GTV;
	pHalFunc->get_tx_power_index_handler = &PHY_GetTxPowerIndex_8188GTV;

	pHalFunc->hal_dm_watchdog = &rtl8188gtv_HalDmWatchDog;
	pHalFunc->SetBeaconRelatedRegistersHandler = &rtl8188gtv_SetBeaconRelatedRegisters;


	pHalFunc->run_thread = &rtl8188gtv_start_thread;
	pHalFunc->cancel_thread = &rtl8188gtv_stop_thread;

	pHalFunc->read_bbreg = &PHY_QueryBBReg_8188GTV;
	pHalFunc->write_bbreg = &PHY_SetBBReg_8188GTV;
	pHalFunc->read_rfreg = &PHY_QueryRFReg_8188GTV;
	pHalFunc->write_rfreg = &PHY_SetRFReg_8188GTV;

	pHalFunc->read_wmmedca_reg = &rtl8188gtv_read_wmmedca_reg;
	...
	pHalFunc->sreset_xmit_status_check = &rtl8188gtv_sreset_xmit_status_check;
	pHalFunc->sreset_linked_status_check  = &rtl8188gtv_sreset_linked_status_check;
...
#endif
	pHalFunc->hal_notch_filter = &hal_notch_filter_8188gtv;

	pHalFunc->c2h_handler = c2h_handler_8188gtv;

	pHalFunc->fill_h2c_cmd = &FillH2CCmd8188GTV;
	pHalFunc->fill_fake_txdesc = &rtl8188gtv_fill_fake_txdesc;
	pHalFunc->fw_dl = &rtl8188gtv_FirmwareDownload;
	pHalFunc->hal_get_tx_buff_rsvd_page_num = &GetTxBufferRsvdPageNum8188GTV;
}
