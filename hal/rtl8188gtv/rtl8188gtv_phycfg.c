...
/**
 * Function:	phy_RFSerialWrite_8188GTV
 *
...
)
{
	...
	Original_Value = phy_RFSerialRead_8188GTV(Adapter, eRFPath, RegAddr);
...
)
{
	...
	phy_RFSerialWrite_8188GTV(Adapter, eRFPath, RegAddr, Data);
}
 ...
 *
 *---------------------------------------------------------------------------*/
s32 PHY_MACConfig8188GTV(PADAPTER Adapter)
{
...
)
{
	...
	/* Config BB and AGC */
	rtStatus = phy_BB8188gtv_Config_ParaFile(Adapter);
...
)
{
	...
	/* RF config */
	/* */
	rtStatus = PHY_RF6052_Config8188GTV(Adapter);

	/*PHY_BB8188GTV_Config_1T(Adapter); */
...
s32 phy_set_txpwr_offload_8188gtv(_adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
...
#ifdef CONFIG_FW_OFFLOAD_SET_TXPWR_IDX
	if (phy_set_txpwr_idx_offload(adapter))
		phy_set_txpwr_offload_8188gtv(adapter);
#endif
}

/* A workaround to eliminate the 2400MHz, 2440MHz, 2480MHz spur of 8188GTV. (Asked by David.) */
void
phy_SpurCalibration_8188GTV(
		PADAPTER					pAdapter,
		u8						ToChannel,
		u8						threshold
)
{
	...
	/* RTW_INFO("===> phy_SpurCalibration_8188GTV: Channel = %d\n", ToChannel); */
...
void
phy_SetRegBW_8188GTV(
		PADAPTER		Adapter,
	enum channel_width	CurrentBW
)
{
	u16	RegRfMod_BW, u2tmp = 0;
	RegRfMod_BW = rtw_read16(Adapter, REG_TRXPTCL_CTL_8188GTV);

	switch (CurrentBW) {
	case CHANNEL_WIDTH_20:
		rtw_write16(Adapter, REG_TRXPTCL_CTL_8188GTV, (RegRfMod_BW & 0xFE7F)); /* BIT 7 = 0, BIT 8 = 0 */
		break;

	case CHANNEL_WIDTH_40:
		u2tmp = RegRfMod_BW | BIT7;
		rtw_write16(Adapter, REG_TRXPTCL_CTL_8188GTV, (u2tmp & 0xFEFF)); /* BIT 7 = 1, BIT 8 = 0 */
		break;

	case CHANNEL_WIDTH_80:
		u2tmp = RegRfMod_BW | BIT8;
		rtw_write16(Adapter, REG_TRXPTCL_CTL_8188GTV, (u2tmp & 0xFF7F)); /* BIT 7 = 0, BIT 8 = 1 */
		break;

	default:
		RTW_INFO("phy_PostSetBWMode8188GTV():	unknown Bandwidth: %#X\n", CurrentBW);
		break;
	}
}

u8
phy_GetSecondaryChnl_8188GTV(
		PADAPTER	Adapter
)
{
...
)
{
	...
	case CHANNEL_WIDTH_40:
		...
		*/
		...
		SubChnlNum = phy_GetSecondaryChnl_8188GTV(Adapter);
		phy_set_mac_reg(Adapter, REG_DATA_SC_8188GTV, BIT3 | BIT2 | BIT1 | BIT0, SubChnlNum);	/* txsc_20 */
		phy_set_mac_reg(Adapter, REG_RRSR_8188GTV, BIT22 | BIT21, 0x0);							/* RRSR_RSC */

		if (0)
			RTW_INFO("%s: REG_DATA_SC_8188GTV(%d) nCur40MhzPrimeSC(%d)\n", __func__, SubChnlNum, pHalData->nCur40MhzPrimeSC);
		break;
	...
	PHY_RF6052SetBandwidth8188GTV(Adapter, pHalData->current_channel_bw);
}
...
)
{
	...
	/* RTW_INFO("===>phy_SwChnl8188GTV: Channel = %d\n", channelToSW); */

	phy_SpurCalibration_8188GTV(pAdapter, channelToSW, 0x16);
}

void
phy_SwChnlAndSetBwMode8188GTV(
	PADAPTER		Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
	...
	if (pHalData->bSwChnl) {
		phy_SwChnl8188GTV(Adapter);
		pHalData->bSwChnl = _FALSE;
	}

	if (pHalData->bSetChnlBW) {
		phy_PostSetBwMode8188GTV(Adapter);
...
void
PHY_HandleSwChnlAndSetBW8188GTV(
		PADAPTER			Adapter,
...
)
{
	...
	/* RTW_INFO("=> PHY_HandleSwChnlAndSetBW8188GTV: bSwitchChannel %d, bSetBandWidth %d\n", bSwitchChannel, bSetBandWidth); */
	/* RTW_INFO("=> %s: ChnlWidth %d\n", __func__, ChnlWidth); */

	/*check is swchnl or setbw */
	if (!bSwitchChannel && !bSetBandWidth) {
		RTW_INFO("PHY_HandleSwChnlAndSetBW8188GTV:  not switch channel and not set bandwidth\n");
		return;
	}
	...
	if (!pHalData->bSetChnlBW && !pHalData->bSwChnl) {
		/* RTW_INFO("<= PHY_HandleSwChnlAndSetBW8188GTV: bSwChnl %d, bSetChnlBW %d\n", pHalData->bSwChnl, pHalData->bSetChnlBW); */
		return;
	}
	...
	if (!RTW_CANNOT_RUN(Adapter))
		phy_SwChnlAndSetBwMode8188GTV(Adapter);
	else {
		if (pHalData->bSwChnl) {
...
)
{
	...
	PHY_HandleSwChnlAndSetBW8188GTV(Adapter, _TRUE, _TRUE, channel, Bandwidth, Offset40, Offset80, channel);
